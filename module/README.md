...
> **息屏冻结（260825 新增）**：独立于恢复充电的省电功能——把非豁免三方应用切为可冻结态，息屏后由 PowerKeeper 冻结/清理（B 站等默认不豁免，直接解决夜间偷跑耗电）；「即时恢复」一键解除冻结（重启 PowerKeeper 秒级生效）。豁免 = 系统电池白名单 + `screen_off_freeze_whitelist`（音乐应用必须豁免）。
